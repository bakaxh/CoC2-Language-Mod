(() => {
  ...
  const t = (s) => {
    ...
    s = s.replace(/&nbsp;/g, " ");
    if (window.pc?.name) {
      if (s.trim() === "Unnamed") return window.pc.name;
      s = s.replace("Unnamed", window.pc.name);
    }
    ...
    const waitTextify = setInterval(() => {
      if (window.textify) {
        ...
        window.textify = function (...args) {
          const originalResult = origTextify.apply(this, args);
        ...
        verbose("[i18n] textify hooked (post-process)");
      }
    }, 50);
